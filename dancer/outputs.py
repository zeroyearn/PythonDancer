...
Intiface support is deliberately optional so the default desktop bundle stays
focused on TCode devices. Install ``python-dancer[intiface]`` to enable it.
"""
...
def _imports():
    try:
        ...
        raise IntifaceUnavailable("Intiface output requires the optional 'buttplug' package. Install python-dancer[intiface].") from exc
...
def intiface_frames(plan: Mapping[str, Sequence[tuple[float, float]]]):
    """Create generic position/intensity frames independent of the Buttplug package."""
    ...
    for i, at in enumerate(times):
        ...
        if previous is None:
            ...
            deltas = [abs(sample_axis(plan.get(axis, ()), at, 50.0) - sample_axis(plan.get(axis, ()), previous[0], 50.0)) for axis in plan]
...
async def play_plan_intiface(
    ...
    *,
    ...
    POSITION_WITH_DURATION receives L0. VIBRATE receives a normalized six-axis
    motion intensity. ROTATE receives normalized R0 excursion. Unsupported
    ...
    try:
        ...
        for at, duration_ms, position, intensity, rotation in frames:
            ...
            if device.has_output(OutputType.POSITION_WITH_DURATION):
                await device.run_output(DeviceOutputCommand(OutputType.POSITION_WITH_DURATION, position, duration=max(20, int(duration_ms / speed))))
