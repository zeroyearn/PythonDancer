...
def safe_normalize(values: Iterable[float]) -> np.ndarray:
    ...
    Constant (or entirely non-finite) input becomes an array of zeros. Non-finite
    values in otherwise valid input are replaced with the finite minimum before
    normalization so a single bad frame cannot poison the whole motion plan.
    """
...
def _aggregate_to_beats(values: np.ndarray, beat_frames: np.ndarray) -> np.ndarray:
    """Aggregate frame values into intervals ending at each beat."""
...
def extract_audio_features(
    ...
    *,
    ...
    """Extract beat-aligned rhythmic and spectral features from mono audio."""
    ...
    if plp and onset.size:
        beat_envelope = librosa.beat.plp(
            onset_envelope=onset,
            sr=sr,
            hop_length=hop_length,
        )
